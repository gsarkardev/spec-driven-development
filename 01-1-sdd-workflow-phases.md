# Spec-driven development workflow phases
...
- **Architecture overview:** A high-level view of how components interact. For the RSS Feed Reader:
    ...
    "Build a Go application with a local SQLite database for storing feeds and articles. Implement a `FeedService` responsible for RSS/Atom feed validation, fetching, and parsing. The application UI should display a list of subscribed feeds and an article reader view with read/unread tracking. When a user adds a feed, the app validates the URL, retrieves the feed content, parses articles, and persists both feed metadata and articles locally in SQLite."
...
- **Technology stack and key decisions:** Explicit documentation of technology choices and rationales. Each decision should align with both specification requirements and organizational principles.
    ...
    - Runtime: Go 1.22+ application.
    - Data storage: SQLite using `modernc.org/sqlite` or `github.com/mattn/go-sqlite3` with `sqlc`, `gorm`, or standard `database/sql` for local persistence.
    - Feed parsing: `github.com/mmcdole/gofeed` for RSS and Atom feed support.
    - Security: HTML sanitization before rendering article content using `github.com/microcosm-cc/bluemonday`.
...
2. Write code to implement the task (manually or using AI assistance).
3. Test that the task meets spec and plan expectations.
4. Mark the task complete and move to the next task.
...
The end state is a fully implemented app or feature where the code aligns with a spec, and you have documentation (spec and plan) that exactly matches the implementation.
